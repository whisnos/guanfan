layui.define(['table', 'form', 'layer'], function (exports) {
    ...
    table.render({
      ...
      , cols: [[
        ...
        , { field: 'status', title: '状态', templet: '#dongtai_status_Tpl', minWidth: 50, align: 'center' }
    ...
    table.on('tool(LAY-cms-content-dongtai-table-list)', function (obj) {
        ...
        } else if (obj.event === 'play_dongtai_video') {
          // 播放视频
          admin.popup({
              title: '播放视频'
              , area: ['360px', '660px']
              , id: 'LAY-popup-dongtai-paly-video'
              , success: function (layero, index) {
                  view(this.id).render('app/common/videoplay', data).done(function(){
                  });
              }
          });
