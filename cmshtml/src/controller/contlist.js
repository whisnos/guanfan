...
layui.define(['table', 'form'], function(exports){
  ...
  table.render({
    ...
    ,url: '/api/banner/list' //模拟接口
    ,cols: [[
      {type: 'checkbox', fixed: 'left'}
      ,{field: 'id', width: 100, title: 'id'}
      ,{field: 'title', title: '海报标题', minWidth: 100}
      ,{field: 'bannerimg', title: '海报图片', templet: '#hbimgViewTpl', align: 'center', maxWidth:'100%', maxHeight:'100%'}
      ,{field: 'linkurl', title: '海报地址', width: 300}
      ,{field: 'recipeid', title: '食谱ID', width: 100}
      ,{field: 'type', title: '类型', templet: '#banner_type_Tpl', align: 'center'}
      ,{field: 'channel', title: '频道', templet: '#banner_channel_Tpl', align: 'center'}
    ...
    if(obj.event === 'del'){
      ...
      // 查看大图
        layer.open({
          ...
          // ,area: ['480px', '640px']
          ,area: ['auto', 'auto']
          ,content: '<div style="text-align: center; padding: 5px; width: 400px; height:400px"><img src="{imgresource}" style="max-width:100%;max-height:100%"></div>'.replace('{imgresource}', layui.setter.basehost + obj.data.bannerimg)
          ...
          ,success: function(layero, index){
              ...
              view(this.id).render('app/common/ossupload', {'operate':2}).done(function(){
                  ...
                  form.on('submit(layuiadmin-app-oss-fileupload)', function(data){
                      ...
                      // console.log(field)
                      if(field.cmsupfiles != ''){
                          document.getElementById('bannerimg').value = field.cmsupfiles.slice(0,-1); // 更新食谱图片文件地址
                      }
                      layer.close(index); //执行关闭 
                  });
              });
