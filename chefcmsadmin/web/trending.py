...
class TrendingEditHandler(BaseHandler):
    ...
    async def post(self):
        ''' 修改热搜 '''
        arg_key = change_byte_to_dict(self.request.body, self.request.query)
        code, msg = await trending_edit(arg_key)
        self.send_cms_msg(code, msg)

class TrendingDelHandler(BaseHandler):
    ''' 删除热搜 '''
    @authenticated
    async def post(self):
        arg_key = change_byte_to_dict(self.request.body, self.request.query)
        code, msg = await trending_del(arg_key)
        self.send_cms_msg(code, msg)


async def trending_add(arg_dict):
    ''' 增加 '''
    trendingid = arg_dict.get('id')
    result = await exists_trending(trendingid)
    if result is False:
        return 2010, "错误的主题数据"
    ...
    insert_result = await dbins.execute(insert_sql, (
        trendingid,
        arg_dict.get('keyword'),
        arg_dict.get('visitCount'),
        arg_dict.get('sort'),
...
async def exists_trending(trendingid):
    ''' 查找热搜词是否存在 '''
    trending_exists_sql = '''select count(id) as ctnum from hot_key where id= ? '''
    exists_result = await dbins.selectone(trending_exists_sql, trendingid)
...
# async def trending_visit_num(trendingid):
#     ''' 返回热搜对应的浏览量 '''
#     result = await exists_trending(trendingid)
#     if result is False:
#         return 0
#     trending_num_sql = '''select count(id) as ctnum from hot_key where id= ? '''
#     num_result = await dbins.selectone(trending_num_sql, trendingid)
#     if num_result is None:
#         return 0
#     else:
#         return num_result.get('ctnum',0)

async def trending_edit(arg_dict):
    ''' 修改 '''
...
async def trending_del(arg_dict):
    ''' 删除 '''
    del_trending_set_sql = '''
    UPDATE hot_keyword
    SET status=-1
    WHERE id = ?
    '''
    del_set_result = await dbins.execute(del_trending_set_sql,
        (
        arg_dict.get('id')
        ))
...
def trending_set_string(arg_dict):
...
async def trending_set(arg_dict):
    ''' 更新 '''
    update_str, upvalue_list = trending_set_string(arg_dict)
    up_sel = '''
    UPDATE hot_key
    SET {}
    where id = ?
...
def trending_search_string(arg_dict):
    ''' 返回搜索条件,arg_dict:所有请求的键值对数据,返回值 (1 where条件语句, 2 where条件对应的值) '''
...
async def trending_list(arg_dict):
    ''' 热搜列表, 页数,每页个数 '''
    ...
    where_str, wvalue_list = trending_search_string(arg_dict)
    # log.warning("{},{}".format(where_str, wvalue_list))
    trending_count_sql = '''select count(1) as ctnum from hot_keyword {}'''.format(where_str)
    b_cnum = await dbins.selectone(trending_count_sql, wvalue_list)
    ...
    blist = await dbins.select(trending_list_sql, wvalue_list)
...
if __name__ == '__main__':
    ...
    async def test_trending_list():
        # res = await trending_list(1,10)
        res = await trending_list({
            'page': '1',
            'limit': '10'})
        print(res)
    ...
    async def test_sql_like():
        # 测试 like 模糊搜索
        trending_count_sql = '''
        select id from hot_key where title like ?
        '''
        res = await dbins.selectone(trending_count_sql, ("%我的新热搜%"))
        print(res)

    async def test_trending_recipe_list():
        res = await trending_recipe_list({'page':1,'limit':100,'id':1})
        print(res)

    async def test_trending_set():
        res = await trending_set({"id":6, "status":0})

    import asyncio
    loop = asyncio.get_event_loop()
    loop.run_until_complete(test_trending_set())
