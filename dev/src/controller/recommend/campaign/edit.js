layui.define(['form', 'layer'], function(exports){
    ...
    $(document).on('click', '#uploadfile_openprize', function(){
        admin.popup({
            title: '上传开奖按钮'
            ,area: ['550px', '450px']
            ,id: 'LAY-popup-recommend-campaign-oss-fileupload-openimg'
    ...
    $(document).on('click', '#uploadfile_joinbutton', function(){
        admin.popup({
            title: '上传参与按钮'
            ,area: ['550px', '450px']
            ,id: 'LAY-popup-recommend-campaign-oss-fileupload-joinimg'
            ,success: function(layero, index){
                updata_set = {
                    'spaceid':document.getElementById('id').value,
                    'operate':7,
    ...
    $(document).on('click', '#uploadfile_mainimg', function(){
        admin.popup({
            title: '上传封面'
            ,area: ['550px', '450px']
            ,id: 'LAY-popup-recommend-campaign-oss-fileupload-mainimg'
            ,success: function(layero, index){
                updata_set = {
                    'spaceid':document.getElementById('id').value,
                    'operate':7,
                    'filename':"main/campaignmain" + new Date().getTime() + ".jpg"
                }; //operate 上传图片类型,1 动态 2 食谱 3 主题 4 海报 5 用户头像 6 高级认证 7 活动 8 其它
    ...
    exports('recommend/campaign/edit', {})
});